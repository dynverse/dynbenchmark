...
## $`r label_metric("edge_flip", "latex")`$ and $`r label_metric("isomorphic", "latex")`$: Number of edge additions and removals to convert one topology to another
...
- All nodes with degree 2 were removed. For example in the topology A ⇨ B ⇨ C ⇨ D, C ⇨ D, the nodes B was removed
...
The different topology based scores are useful for different scenarios:

- If the two trajectories are only comparable when the topology is exactly the same, the $`r label_metric("isomorphic", "latex")`$ should be used.
- If it is important that the topologies are similar (but possible not isomorphic), the $`r label_metric("edge_flip", "latex")`$ is most appropriate.
