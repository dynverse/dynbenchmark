---
title: "Supplementary Note 1: Metrics to compare two trajectories"
output: 
  pdf_document:
...
We compared the three scores for several common topologies (`r ref("fig", "topology_scores_overview")`). While conceptually very different, the $`r label_metric("edge_flip", "latex")`$ and $`r label_metric("him", "latex")`$ still produce similar scores (`r ref("fig", "topology_scores_overview", "b")`). The $`r label_metric("him", "latex")`$ tends to punish the detection of cycles, while the $`r label_metric("edge_flip", "latex")`$ is more harsh for differences in the number of bifurcations (`r ref("fig", "topology_scores_overview", "c")`). The main difference however is that the $`r label_metric("him", "latex")`$ takes into account edge lengths when comparing two trajectories, as illustrated in (`r ref("fig", "topology_scores_overview", "d")`).
...
add_fig(
  ...
  glue::glue("Mapping cells to their closest milestone or branch for the calculation of the ${label_metric('F1_branches', 'latex')}$, ${label_metric('F1_edges', 'latex')}$ and ${label_metric('F1_branches', 'latex')}$ ."),
  "",
  width = 12,
  height = 5
)
...
## $`r label_metric("featureimp_cor", "latex")`$: The accuracy of dynamical differentially expressed features/genes.

Perhaps the main advantages of studying cellular dynamic processes using single-cell -omics data is that the dynamics of gene expression can be studied for the whole transcriptome. This allows the study of dynamic regulatory networks and gene expression modules. Such analyses rely on a "good-enough" cellular ordering, so that the cellular ordering can be used to identify dynamical differentially expressed genes.

To calculate the $`r label_metric("featureimp_cor", "latex")`$ we used Random forest regression to rank all the features according to their importance in predicting the positions of cells in the trajectory. Specifically, we calculated the geodesic distances for each cell to all milestones in the trajectory. We then trained a Random Forest regression model (implemented in the R _ranger_ package [@wright_rangerfastimplementation_2017], [https://github.com/imbs-hl/ranger](https://github.com/imbs-hl/ranger)) to predict these distances for each milestone, based on the expression of genes within each cell. We then extracted feature importances using the Mean Decrease in Impurity (`importance = 'impurity'` parameter of the `ranger` function), as illustrated in (`r ref("fig", "featureimp_overview.rds")`). The overall importance of a feature (gene) was then equal to the mean importance over all milestones.
