...
³ Center for Medical Genetics, Ghent plot_topology_complexity %>% write_rds(result_file("topology_complexity.rdsUniversity Hospital, Ghent, Belgium.  
...
<!-- §4: Our study -->
In this study, we performed a comprehensive evaluation of  →`r n_methods_evaluated`← TI methods (`r ref("fig", "ti_evaluation_overview", "a")`, `r ref("sfig", "overview_evaluation_expanded")`). →We included methods primarily based← on their free availability and presence of a programming interface (`r ref("stable", "tools")`). →We evaluated each method on four core aspects: (i) their accuracy given a gold or silver standard on `r n_datasets_real` real and `r n_datasets_synthetic` synthetic datasets, (ii) the user friendliness of the provided software and documentation, (iii) their scalability with both the number of cells and features, and (iv) the stability of the predictions across multiple bootstrap replicates. Finally, we combined these metrics to provide a set of practical guidelines for users. ←
...
add_fig(
  ...
  caption_text = "a) The methods were characterised according to the most complex trajectory type they can infer, whether or not the inferred topology is constrained by the algorithm or a parameter, and which prior information a method requires or can optionally use. b) The methods are ordered according to the overall score achieved both in the benchmark and QC evaluation. c) Also shown are the aggregated scores per metric, source and trajectory type, as well as the average execution time across all datasets and the percentage of executions in which no output was produced. d) Performance in the quality control evaluation is highly variabl[b]e, even amongst the highest ranked methods according to the benchmark evaluation. Also listed are the quality control scores aggregated according to practicality and the different categories.",
  width = 15,
  height = 20
)
```
...
add_fig(
  fig_path = result_file("overview.pdf", experiment_id = "08-summary"),
  ref_id = "results_overview_detailed", 
  caption_main = glue::glue("Overview of the results on the evaluation of {n_methods_evaluated} TI methods and {n_control_methods_evaluated} control methods."), 
  caption_text = "a) The methods were characterised according to the most complex trajectory type they can infer, whether or not the inferred topology is constrained by the algorithm or a parameter, and which prior information a method requires or can optionally use. b) The methods are ordered according to the overall score achieved both in the benchmark and QC evaluation. c) Also shown are the aggregated scores per metric, source and trajectory type, as well as the average execution time across all datasets and the percentage of executions in which no output was produced. d) Performance in the quality control evaluation is highly variabl[c]e, even amongst the highest ranked methods according to the benchmark evaluation. Also listed are the quality control scores aggregated according to practicality and the different categories.",
  width = 15,
  height = 20
)
```
...
add_fig(
  fig_path = result_file("topology_complexity.rds", experiment_id = "10-topologies"),
  ref_id = "topology_complexity", 
  caption_main = "Comparison of the size (number of nodes and number of edges) of the topology predicted by the top TI methods, compared to the size of the reference trajectory.", 
...
We observed no clear relation between method quality and method performance (`r ref("sig", "results_overview")`). We could also not find any quality aspect which was significantly associated with method performance.
...
Critical to the broad applicability of TI methods is standardisation of the input and output interfaces of TI methods, so that users can effortlessly execute TI methods on their dataset of interest, compare different predicted trajectories, and apply downstream analyses such as find genes important for the trajectory, network inference [@aibar_scenicsinglecellregulatory_2017] or finding modules of genes [@saelens_comprehensiveevaluationmodule_2018]. Our framework is an initial attempt at tackling this problem, and we illustrate its usefulness here by comparing the predicted trajectories of several top-performing methods →on some a linear, a tree, a cyclic and a disconnected graph dataset (`r ref("fig", "example_predictions")`). Using our framework, this figure can be recreated by using only a couple of lines of R code (`r print_url("https://www.github.com/dynverse/dynmethods")`). This framework should still be extended to allow easier downstream analyses. In addition, further discussion within the field is required in order to arrive at a consensus concerning a common interface for trajectory models, which can include additional features such as uncertainty and gene importance.←
...
→ 
The processed real and synthetic datasets used in this study are deposited on Zenodo ([doi.org/10.5281/zenodo.1211533](https://doi.org/10.5281/zenodo.1211533[f])) [@cannoodt_goldstandardsinglecell_]. 
...
[a]De nomenclatuur in deze figuur moet nog aangepast worden aan de nieuwe maten...
[b]Als we zoiets zeggen moeten we misschien ook SDs tonen op de plots ?
[c]Als we zoiets zeggen moeten we misschien ook SDs tonen op de plots ?
[d]Update when benchmarking results
[e]Update when benchmarking results
[f]Update the doi when reuploaded
[g]TODO
[h]I will try to do my best in adding this if you don't do it first Robrecht
[i]I will try to do my best in adding this if you don't do it first Robrecht