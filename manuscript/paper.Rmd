...
→ 
The processed real and synthetic datasets used in this study are deposited on Zenodo ([doi.org/10.5281/zenodo.1211533](https://doi.org/10.5281/zenodo.1211533[d])) [@cannoodt_goldstandardsinglecell_]. 
...
→To simulate certain trajectory topologies, we therefore designed module networks in which the cells follow a particular trajectory topology given certain parameters. Two module networks generated linear trajectories (linear and linear long), one generated a bifurcation, one generated a convergence, one generated a multifurcation (trifurcating), two generated a tree (consecutive bifurcating and binary tree), one generated an acyclic graph (bifurcating and converging), one generated a complex fork (trifurcating), one generated a rooted tree (consecutive bifurcating) and two generated simple graph structures (bifurcating loop and bifurcating cycle). The structure of these module networks is available at[e] `r print_url("https://github.com/dynverse/dyngen/tree/master/inst/ext_data/modulenetworks")`.←
...
To classify the time complexity of each method, we predicted the running time at 10.000 features with increasing number of cells from 100 to 100.000, with steps of 100. A method was classified as constant / low slope if on average an addition of 100 cells would increase the running time with less than 0.1 seconds. To classify as sublinear, linear, quadratic, or superquadratic, we scaled the times between 0 and 1, and classified the method as sublinear if the running time was on average 5% above the diagonal, superlinear if on average 5% under the diagonal, and linear otherwise.


## Stability
In the ideal case, a method should produce a similar trajectory, even when the input data is slightly different. However, running the method multiple times on the same input data would not be the ideal approach to assess the stability of a method, given that a lot of tools artificially are artificially deterministic by resetting the pseudorandom number generator (for example, using the set.seed function in R). To assess the stability of each method, we therefore selected a number of datasets, which consisted of 25% of the datasets accounting for 15% of the total runtime, chosen such that after aggregation the overall scores still has > 0.99 correlation with the original overall ranking. We subsampled each dataset 10 times with 95% of the original cells and 95% of the original features. We ran every method on each of the bootstraps, and assessed the stability by calculating the benchmarking scores between each pair of subsequent models (run $i$ is compared to run $i+1$). For the `r label_metric("correlation")` and `r label_metric("F1_branches")`, we only used the intersection between the cells of two datasets, while the intersection of the features was used for the `r label_metric("featureimp_wcor")`.
←
## Usability
...
W.S., R.C., H.T. and Y.S. designed the study. W.S. and R.C. performed the experiments and analysed the data. W.S., R.C., and H.T. implemented software packages. W.S., R.C., Y.S. and H.T. prepared the manuscript. Y.S. supervised the project.
...
[a](Not shown), or include in Supp Fig 4?
[b]Yeah we don't include this yet, should be in sup fig 4
[c]TODO: Will be added as sup fig
[d]Update when final
[e]the following link renders as verbatim code!