---
title: "Accuracy, scalability, robustness and usability of single-cell trajectory inference methods[a][b]"
...
Methods in which the topology of the model was fixed by the algorithm or by a parameter obtain a much higher stability score. As topology-free methods have more degrees of freedom, the stability of the method is also typically lower ─ with a few exceptions.[e][f]
 
We will discuss each evaluation criterion in more detail, after which we conclude with guidelines for method users and future perspectives for method developers.
```{r}
add_fig(
...
←
## Implementation quality and user friendliness


While not directly related to the accuracy of the inferred trajectory, it is also important to assess the quality of the implementation and how easy it is for a biological user to use [@_doesyourcode_2018]. We scored each method using a transparent checklist of important scientific and software development practices, including software packaging, documentation, automated code testing, and publication into a peer-reviewed journal (`r ref("stable", "qc_scorseheet")`).
...
→ 
The processed real and synthetic datasets used in this study are deposited on Zenodo ([doi.org/10.5281/zenodo.1211533](https://doi.org/10.5281/zenodo.1211533[i])) [@cannoodt_goldstandardsinglecell_]. 
...
Finally, to get an overall benchmarking score, we aggregated the different metrics using a geometric mean.[j][k][l][m]
←
## Method execution
...
[a]?
[b]we changed the title :)
[c]Try to rewrite a little bi
[d]somehing like his?
[e]TODO: rework enumeration into a paragraph
[f]something like this?
[g](Not shown), or include in Supp Fig 4?
[h]Yeah we don't include this yet, should be in sup fig 4
[i]Update when final
[j]This might give the impression of being a bit "overengineered".  Do we also show somewhere how much the resulting ranking using this aggregation would differ from a very simple "mean" ranking of the individual metrics/datasets ?
[k]No we don't do this yet. If we do this, perhaps we should just add a "benchmarking robustness" supplementary note.
[l]The results do change when we simply use a mean, but then they are very biased, eg. because 1/4 of our datasets are linear, the linear methods perform well..
[m]I adapted the text a little bit to better explain the 'why'
[n]Todo once this experiment is finished
[o]Good enough?