...
```{r}
n_tools <- tools %>% filter(type == "algorithm") %>% nrow()
n_methods_evaluated <- nrow(filter(methods, evaluated, type %in% c("algorithm", "algorithm_test")))
n_control_methods_evaluated <- nrow(filter(methods, evaluated, type %in% c("control", "control_test")))
n_datasets_real <- 10
n_datasets_synthetic <- 20
```
# Abstract
...
```{r}
add_sfig(
  fig_path = "figures/overview_evaluation_expanded_v1.svg",
...
<!-- Inclusion criterion -->
Methods were excluded from the evaluation based on several criteria: `r glue::glue_collapse(glue::glue("({non_inclusion_reasons$footnote}) {non_inclusion_reasons$long}"), ", ")`. The discussions on individual methods can be found at `r print_url("https://github.com/dynverse/dynmethods/issues?q=label:unwrappable")` This resulted in the inclusion of  `r n_methods_evaluated` methods in the evaluation.
...
For each method, we wrote a wrapper script based on example scripts or tutorials provided by the authors. This script will read in the input data (as described below), run the method, and output the files required to construct a trajectory (as described below). To wrap each method, we based ourselves on example scripts or tutorials which are mentioned in the comments of each wrapper script. We also created a script to generate an example dataset, which is used to automatically test every method using travis continuous integration (`r print_url("https://travis-ci.org/dynverse")`).


We used the github issues system to contact the authors of each method, and asked for feedback on the wrappers, the metadata, and the quality control. These discussion can be viewed on github (`r print_url('https://github.com/dynverse/dynmethods/issues?q=label:"method+discussion"')`). About ⅓ of authors responded, and we improved the wrappers based on their feedback. 
←
### Method input
...
<!-- Prior information -->
A large portion of the methods requires some form of prior information (e.g. a start cell) in order to be executable. Other methods optionally allow to exploit certain prior information, though this is not required. Prior information can be supplied as a starting cell from which the trajectory will originate, a set of important marker genes, or even a grouping of cells into cell states. Providing prior information to a TI method can be both a blessing and a curse. In one way, prior information can help the method to find the correct trajectory among many, equally likely, alternatives. On the other hand, incorrect or noisy prior information can bias the trajectory towards current knowledge. Moreover, prior information is not always easily available, and its subjectivity can therefore lead to multiple equally plausible solutions, restricting the applicability of such TI methods to well studied systems.
...
conversion <- function(conversion_oi) {
  methods_evaluated %>% 
    filter(type %in% c("algorithm", "algorithm_test")) %>%
...
* **Type 5, cluster assignment:** `r conversion("cluster_graph")`. The method returned a milestone network, and an assignment of each cell to a specific milestone. Cells were positioned onto the milestones they are assigned to.
...
* →**Type 7, branch trajectory[a]:** `r conversion("branch_trajectory")`. The method returned an assignment of cells to a particular branch, a pseudotime for each cell within the branch, and a network connecting these different branch. We converted this branch network to a milestone network by calculating the line graph of the network. The cells were then placed between each two milestones using the provided pseudotime. ←
* **Type 8, cell graph:** `r conversion("cell_graph")`. The method returned a network of cells, 
...
* **Sincell:** To constrain the number of milestones this method creates, we merged two cell clusters iteratively until the percentage of leaf nodes was below a certain cutoff, default at 25%.  →This was then processed as a type 8, cell graph, method.←
...
More information on how each method was wrapped can be found within the comments of each wrapper script.
←
...
For every simulator, we took great care with trying to make the datasets look like real data as  best as possible. To do this, we extracted several parameters from all real datasets. We calculated the number of differentially expressed features within a trajectory using a wilcoxon test between every pair of cell groups. We also calculated several other parameters, such as drop-out rates and library sizes using the Splatter package [@zappia_splattersimulationsinglecell_2017]. These parameters were then given to the simulators when applicable, as described below.
←
### dyngen


The workflow to generate synthetic data is based on the well established workflow used in the evaluation of network inference methods [@schaffter_genenetweaversilicobenchmark_2011; @marbach_wisdom_2012] and consists of four main steps: network generation, simulation, gold standard extraction and simulation of the scRNA-seq experiment. At every step, we tried to mirror real regulatory networks, while keeping the model simple and easily extendable. For every synthetic dataset, we used a random real dataset as a reference dataset (from those described earlier), making sure the number of variable genes and cells were similar.
#### Network generation
...
Next, we created a set of rules to which we think a good trajectory metric should conform, and tested this empirically for each metric by comparing scores before and after perturbing a dataset (`r ref("snote", "metrics")`). Based on this analysis, we chose 4 metrics for ←
...
```{r}
add_sfig(
  ggplot(),#figure_file("aspects_v2.svg", experiment_id="manual_figures/metrics"), 
  "score_aspects", 
...
```{r}
add_sfig(
  ggplot(),#figure_file("overview_aggregation_v1.svg", experiment_id="manual_figures/metrics"), 
...
[a]Gotta update the figure to reflect this