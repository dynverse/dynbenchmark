...
In this study, we performed a comprehensive evaluation of  `r n_methods_evaluated` TI methods (`r ref("fig", "ti_evaluation_overview", "a")`, `r ref("sfig", "overview_evaluation_expanded")`). We included methods primarily based on their free availability and presence of a programming interface (`r ref("stable", "tools")`). In order to make the outputs from different methods directly comparable to each other, we developed a common probabilistic model for representing trajectories from all possible sources (`r ref("fig", "ti_evaluation_overview", "b")`). In this model, the overall topology is represented by a network of "milestones", and the cells are placed within the space formed by each set of connected milestones. The output of each method was converted to this model using different wrapper types (`r ref("fig", "results_summary", "a")`).


When required, we also provided prior information to the method, which ranges from "soft" priors, such as start or end cells, to "harder" priors such as the known grouping of the cells (`r ref("fig", "results_summary", "a")`).
...
Each method was evaluated on four core aspects: (i) benchmarking given a gold or silver standard on `r n_datasets_real` real and `r n_datasets_synthetic` synthetic datasets, (ii) their scalability with both the number of cells and features (iii) the stability of the predictions across multiple bootstrap replicates, and (iv) the user friendliness of the provided software and documentation. Overall, we found a large diversity across all four evaluation criteria, with only a few methods, such as PAGA, GNG, Slingshot, Projected TSCAN and SCORPIUS, performing well across the board  (`r ref("fig", "results_summary", "b")`). We will first discuss each evaluation criterion in detail, after which we conclude with guidelines for method users and future perspectives for method developers.
```{r}
add_fig(
  ...
  caption_text = "(a) We characterised the methods according to their required priors, the types of inferreable topologies, whether or not the inferred topology is constrained by the algorithm or a parameter. The methods are grouped vertically based on the most complex trajectory type they can infer. (b) Overall results of the evaluation on four criteria: the benchmarking using a reference trajectory on real and synthetic data, the scalability with increasing number of cells and features, the stability across dataset bootstraps, and the quality control of the tool which implemented the method. ",
  width = 15,
  height = 18
)
```
...
add_sfig(
  ...
  caption_main = "Robustness of the benchmark", 
...
We observed no clear relation between method quality and method performance (`r ref("fig", "results_summary")`). We could also not find any quality aspect which was significantly associated with method performance.
...
Next, we calculated for each metric, trajectory type and dataset source an average score using an arithmetic mean.
...
Next, we aggregated the scores across trajectory types and dataset source (real/gold, real/silver, synthetic/dyngen, synthetic/splatter, synthetic/prosstt and synthetic/dyntoy), so that each of these had an equal weight on the final score:


1. Calculate a score for each method, dataset source and trajectory type
←
## Benchmark
### Method execution
