...
tools$publication_date <- tools$publication_date + runif(length(tools$publication_date), 0, 1)
...
publication_cumulative_by_type_interpolated <- publication_cumulative_by_type_interpolated %>%
  ...
  mutate(n_tools_cumulative = lag(n_tools_cumulative, 1), publication_date = publication_date - 0.1, counter = 0, prefix = TRUE) %>%
...
n_tools_over_time <- publication_cumulative_by_type_interpolated %>%
  ...
  scale_y_continuous(label_long("n_tools"), expand = c(0, 0), limits = c(0, nrow(tools)+2)) +
...
##  ............................................................................
##  Platforms                                                               ####
platforms <- tools %>% separate_rows(platform = platforms, sep = ", ") %>%
  group_by(platforms) %>%
...
trajectory_components <- tools %>% filter(!is.na(date))
