...
experiment("06-optimise_parameters/5-plot_all_trajectories")
...
datasets <- map_df(paste0(local_datasets_folder, "/", dataset_ids, ".rds"), read_rds) %>% filter(dataset_source != "toy")
methods <- dynwrap::get_ti_methods()
method_names <- methods$short_name %>% discard(~ . %in% c("identity", "shuffle", "random", "manual"))

walk(datasets$id, function(tid) {
  dir.create(derived_file(tid), recursive = T, showWarnings = F)

  dataset <- datasets %>% filter(id == tid) %>% extract_row_to_list(1)

  cat("Processing ", tid, "\n", sep = "")
  walk(method_names, function(mid) {
    method <- methods %>% filter(short_name == mid)

    debug_out <- outputs_ind %>% filter(method_short_name == mid, dataset_id == tid) %>% arrange(desc(rank_correlation))
    ...
    walk(seq_len(nrow(debug_out)), function(i) {
      tryCatch({
        ...
        model <- load_dyneval_model(method_short_name = mid, model_id = model_id, experiment_id = "06-optimise_parameters/3-evaluate_parameters")[[1]]

        wp_cells <- unique(c(model$waypoint_cells, dataset$waypoint_cells))
        dataset_geo <- dynwrap::compute_tented_geodesic_distances(dataset, wp_cells)
        model_geo <- dynwrap::compute_tented_geodesic_distances(model, wp_cells)

        # g_geo <-
        #   if (dataset$dataset_source != "real") {
        #     ggplot() + geom_jitter(aes(as.vector(dataset_geo), as.vector(model_geo)), width = .05, height = 0.05, alpha = .5)
        #   } else {
        #     ggplot() + geom_violin(aes(factor(as.vector(dataset_geo)), as.vector(model_geo)))
        #   }
        #
        ...
        g_traj <- cowplot::plot_grid(
          plot_default(dataset) + labs(title = "Task"),
          plot_default(model) + labs(title = "Prediction"),
          g_own + labs(title = "Method's own plot"),
          plot_combined(model, dataset) + labs(title = "Task topology\nwith prediction colouring"),
          plot_combined(dataset, model) + labs(title = "Prediction topology\nwith dataset colouring"),
          nrow = 2
        )

        g_strip <- plot_strip_connections(dataset, model)
