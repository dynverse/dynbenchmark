...
generate_model <- function(params) {
  ...
  saveRDS(model, dataset_source_file(glue::glue("{params$settings$params_i}_model.rds")))
}
...
simulate <- function(params) {
  ...
  saveRDS(simulation, dataset_source_file(glue::glue("{params$settings$params_i}_simulation.rds")))
}
...
check_simulation <- function(params, params_i = params$settings$params_i) {
  ...
  simulation_location <- dataset_source_file(glue::glue("{params_i}_simulation.rds"))
...
extract_goldstandard <- function(params) {
  ...
  saveRDS(gs, dataset_source_file(glue::glue("{params$settings$params_i}_gs.rds")))
}
...
generate_experiment <- function(params) {
  ...
  saveRDS(experiment, dataset_source_file(glue::glue("{params$settings$params_i}_experiment.rds")))
}
...
normalise <- function(params) {
  ...
  graphics.off()
  pdf(dataset_source_file(glue::glue("{params$settings$params_i}_normalisation_plot.pdf")), width = 12, height = 12)
  dev.control('enable')
  ...
  saveRDS(normalisation, dataset_source_file(glue::glue("{params$settings$params_i}_normalisation.rds")))
...
plot_model <- function(params) {
  ...
  model_plot_location <- dataset_source_file(glue::glue("{params$settings$params_i}_model_plot.pdf"))
...
plot_simulation <- function(params) {
  ...
  simulation_plot_location <- dataset_source_file(glue::glue("{params$settings$params_i}_simulation_plot.pdf"))
...
plot_goldstandard <- function(params) {
  ...
  gs_plot_location <- dataset_source_file(glue::glue("{params$settings$params_i}_gs_plot.pdf"))
...
plot_experiment <- function(params) {
  ...
  experiment_plot_location <- dataset_source_file(glue::glue("{params$settings$params_i}_experiment_plot.pdf"))
...
wrap <- function(params) {
  ...
  dataset <- dyngen::wrap_dataset(params, data$model, data$simulation, data$gs, data$experiment, data$normalisation)
  dataset$uuids <- list(dataset = uuid::UUIDgenerate()) %>% c(data$normalisation$uuids)

  dataset_location <- dataset_source_file(glue::glue("{params$settings$params_i}_dataset.rds"))
  dataset %>% saveRDS(dataset_location)
}
