...
```{r}
add_sfig(
  figure_file("methods_timeline.svg", "4-method_characterisation"),
  "methods_over_time", 
...
A plethora of TI methods has been developed over the last years, and even more are being created every month (`r ref("sfig", "methods_over_time", "a")`). We gathered a list of `r sum(implementations$is_ti, na.rm=TRUE)` TI methods (`r ref("table", "implementations")`), which we believe to be the most complete list to date. It is perhaps surprising that almost all of them have a unique combination of characteristics, in terms of the required inputs (prior information), produced outputs (topology fixing and trajectory type) and methodology used.
...
add_table(
  ...
  "Overview of current trajectory inference methods, and whether they were included in this current evaluation study."
)
```
...
Early TI methods typically fixed the topology algorithmically (e.g. linear [@bendall_singlecelltrajectorydetection_2014; @schlitzer_identificationcdc1cdc2committed_2015; @shin_singlecellrnaseqwaterfall_2015; @campbell_bayesiangaussianprocess_2015] or bifurcating [@haghverdi_diffusionpseudotimerobustly_2016; @setty_wishboneidentifiesbifurcating_2016]) or through parameters provided by the user [@trapnell_dynamicsregulatorscell_2014; @matsumoto_scoupprobabilisticmodel_2016]. The focus of these methods was therefore mainly to correctly order the cells along this fixed topology. 
Other methods attempt to infer the topology computationally, which increases the difficulty of the problem at hand, but allows a method to be more broadly applicable on more use cases. Methods that perform topology inference are still in the minority, though current trends suggest this will soon change (`r ref("sfig", "methods_over_time", "c")`). Most noticeable of these methods is AGA[@wolf_graphabstractionreconciles_2017], which is the only TI method currently able to predict disconnected graphs. 


<!-- §4: Prior information -->
Another key characteristic is the selection of prior information that a method requires or can optionally exploit. Examples of prior informations are supplying a starting cell from which the trajectory will originate, a set of important marker genes, or even a grouping of cells into cell states. Giving prior information to a TI method can be both a blessing and a curse. In one way, prior information can help the method to find the correct trajectory among many, equally likely, alternatives. On the other hand, incorrect or noisy prior information can bias the trajectory towards current knowledge. Moreover, prior information is not always easily available, and its subjectivity can therefore lead to multiple equally plausible solutions, limiting the applicability of such TI methods to only well studied systems.


<!-- §5: Components -->
A reductionist approach to characterising TI methods is dissecting it into a set of algorithmic components, as any one component can have a significant impact on the performance, scalability, and output data structures. Across all TI methods, these components can be broadly grouped into two stages; (i) conversion to a simplified representation using dimensionality reduction, clustering or graph building and (ii) ordering the cells along the simplified representation [@cannoodt_computational_2016]. Interestingly, components are frequently shared between different algorithms (`r ref("sfig", "implementation_components")`). MST in particular, used in the first single-cell RNA-seq trajectory inference methods [@trapnell_dynamicsregulatorscell_2014], is used in almost half of all methods we evaluated  (`r ref("sfig", "implementation_components", "b")`).


```{r}
add_sfig(
  figure_file("implementation_components.svg","4-method_characterisation"), 
...
```{r}
add_sfig(
  figure_file('synthetic_data_generation.svg', "2-dataset_characterisation/3-synthetic"),
...
We not only assessed the accuracy of the predictions made by a trajectory, but also investigated the quality of the method's implementation. To do this, we scored each method using a checklist of important scientific and software development practices, including software packaging, documentation, automated code testing, and peer review. This allowed us to rank each method based on its user friendliness, developer friendliness, and potential broad applicability on new unseen datasets. Finally, using both the benchmark results and quality control, we finally present a flow chart with practical guidelines for selecting the most appropriate TI method for a given use case.


`r anchor("fig", "evaluation_overview")`
![](`r figure_file("ti_evaluation_overview_v3.svg", experiment_id="evaluation_overview")`)


`r ref("fig", "evaluation_overview")` **An overview of several key aspects of the evaluation.** a) . b) In order to make any two trajectories comparable with one another, a common trajectory model was used to represent gold standard trajectories from the real and synthetic datasets, as well as any predicted trajectories from TI methods. c) Three metrics were defined in order to assess the similarity in cell ordering, cell neighbourhood, and topology, for any two trajectories.


```{r}
add_sfig(
  figure_file('expanded_ti_evaluation_overview_v1.svg', 'evaluation_overview'),
...
![](`r figure_file("overview.svg", experiment_id="9-main_figure")`)
...
Ordering all methods by their overall benchmarking score, we found that Slingshot predicts the most accurate trajectories, followed by TSCAN and Monocle DDRTree. Observing the benchmark scores per trajectory type, Slingshot seems to be the only method that performs well across most trajectory types. However, several methods are seemingly specialised in predicting specific trajectory types; for example SCORPIUS for linear trajectories, reCAT for cycles, Monocle DDRTree for rooted trees, and, surprisingly Wishbone for multifurcations. We will discuss the effect of topology types and constraints on inferrable trajectory types placed by the method in Section SEC_TRAJTYPE. 


<!-- §3: real versus synthetic -->
We observed a high correlation (0.7-0.9) between results originating from real datasets versus those originating from synthetic datasets (`r ref("sfig", "real_synthetic_comparison")`). This confirms both the relevance of the synthetic data and the accuracy of the gold standard in the real datasets. However, this correlation was lower for converging and multifurcating datasets, potentially because only a small number of real datasets were available which contain such topologies (`r ref("stable", "datasets")`).


```{r}
add_sfig(
  figure_file(
    "real_synthetic_comparison.svg",
    "5-optimise_parameters/5-real_synthetic_comparison"
  ), 
  "real_synthetic_comparison", 
  "Comparison between performance on real and synthetic datasets across trajectory types."
)
```


<!-- §4: Compare metrics -->
As the different metrics were selected to assess the correctness of a trajectory in various approaches, it is expected to observe differences in rankings of TI methods amongst the different metrics. While we saw no direct link between the edge flip scores versus the correlation or RF MSE metric, methods that obtain a high correlation score also tended to obtain higher RF MSE scores (`r ref("sfig", "metric_comparison")`). In addition, methods able to detect more complex trajectory types seemed to obtain higher edge flip scores, in comparison to methods whose trajectory topology was fixed to a simple trajectory topology. We will explore this issue in more detail in a further section SEC_TRAJTYPE.
...
```{r}
add_sfig(
  figure_file('metrics_comparison.svg', "5-optimise_parameters/7-metric_comparison"), 
  "metric_comparison", 
  "Comparison between the different metrics across trajectory types."
)
```
...
During the execution of a method on a dataset, if the time limit (>6h) or memory limit (32GB) was exceeded (>6h) or an error was produced, a zero score would be returned for that execution. If a method consistently generated errors on this dataset across all replicates, the error is called “data-specific”, otherwise “stochastic”. Several methods obtained a high overall score despite having 5 to 10% of failed executions (e.g. Monocle DDRTree, SLICE, Wishbone), meaning these methods could rank even higher if not for the failed executions. Methods that do not scale well with respect to the number of cells or genes will exceed the time or memory limits for the largests of datasets (reCAT, SCOUP, StemID). For a few methods, the time or memory limits were exceeded to frequently that insufficient results were generated to compare it to other methods (SCIMITAR, Ouija, Pseudogp).




Comparing the maximum overall score over time, it is encouraging to see multiple incremental improvements over the state-of-the-art (`r ref("sfig", "leading_method")`). We hope the identifications of the main strengths and weaknesses of existing TI methods will lead to the next few state-of-the-art improvements.
...
```{r}
add_sfig(
  figure_file('leading_method.svg', "9-main_figure"), 
  "leading_method", 
...
In most cases, methods which can handle a particular trajectory type also performed better on data containing this particular trajectory type (`r ref("sfig", "trajtype_handle_comparison")`). These methods had typically better edge flip scores - as can be expected - and RF MSE scores compared to methods not able to handle the particular trajectory type. However, the correlation score typically followed the opposite pattern, where methods restricted to linear trajectory types, such as embeddr, SCORPIUS and phenopath, produced the best ordering, irrespective of whether the dataset contained a linear trajectory or more complex trajectories.
To further investigate, we divided all methods in two groups: linear methods (restricted to linear and cyclic topologies) and non-linear methods. While there were no significant differences on linear datasets, non-linear methods had significantly higher edge flip scores but lower correlation scores on datasets containing more complex trajectory types  (`r ref("sfig", "linear_split_comparison")`). Together, this indicates that current non-linear methods potentially contain less accurate ordering algorithms. A combination of the ordering methods from the top linear methods, combined with the topology inference from top non-linear methods, could therefore be a possibility for future research. 


```{r}
add_sfig(
  figure_file('trajtype_handle_comparison.svg', "8-compare_topology"), 
...
```{r}
add_sfig(
  figure_file('linear_split_comparison.svg', "8-compare_topology"), 
  "linear_split_comparison", 
  "Linear methods have a higher correlation score on non-linear datasets. Methods were split in linear methods (those which can either only detect cyclic or linear trajectories) and non-linear methods."
)
```


Despite their similar overall performance, the topologies predicted by the top methods differed considerably. Trajectories detected by the default parameters of slingshot and cellTree tended to be simpler, while those detected by TSCAN and Monocle DDRTree gravitated towards more complex topologies (`r ref("fig", "topology_complexity_comparison")`). Monocle DDRTree for example, frequently predicted a tree topology, even when only cyclic, linear or bifurcating topology was present in the data (`r ref("fig", "topology_complexity_comparison", "a")`). Trajectories generated by Monocle DDRTree (at default parameters) tend to contain more nodes and edges (`r ref("fig", "topology_complexity_comparison", "b")`), which could give this method an edge on datasets with complex tree topologies, but could also explain its relatively low performance on linear and bifurcating datasets. Indeed, when we assessed how often a method can infer the correct topology, slingshot and TSCAN were good at correctly predicting linear, bifurcating and converging trajectories, while Monocle DDRTree was by far the best method to infer tree topologies. Nonetheless, the overall accuracy of topology prediction was very low, with Slingshot correctly predicting bifurcating and converging topologies for half of the datasets, and Monocle DDRTree predicting the correct tree topology in 12% of the cases (`r ref("sfig", "topology_sensitivity")`). Therefore, inferring the correct topology without parameter tuning is still an open challenge. Conversely, when the data contains a complex trajectory structure, TI currently will still require a considerable guidance by the end user, to either optimise the parameters or to choose the method with which the output best fits the user's expert knowledge.


`r anchor("fig", "topology_complexity_comparison")`
![](`r figure_file("topology_complexity_comparison.svg","8-compare_topology")`)


`r ref("fig", "topology_complexity_comparison")` **INSERT CAPTION.** Caption subdetails.


```{r}
add_sfig(
  figure_file('topology_sensitivity.svg', "8-compare_topology"), 
...
In the current version of the evaluation, we only provided prior information when it was required by a method. We did not observe a major difference in method performance between methods which did and did not receive prior information (`r ref("sfig", "required_priors_performance_comparison")`). Rather, methods who received prior information were on average positioned in the middle of the ranking.  Furthermore, we could not find any dataset where methods which received prior information performed significantly better than other methods.
...
```{r}
add_sfig(
  figure_file('required_priors_performance_comparison.svg', "10-prior_information"), 
...
add_stable(
  ...
  "Importance values for algorithmic components."
)
```
...
add_stable(
  ...
  "qc_checks", 
  "Scoring scheme for method quality control."
)
```
...
![](`r figure_file("implementation_qc_overview.svg","4-method_characterisation")`)


`r ref("fig", "implementation_qc_overview")` **Overview of the quality control results for every method.** 


```{r}
add_sfig(
  figure_file("qc_over_time.svg","4-method_characterisation"), 
  "qc_over_time", 
...
add_sfig(
  figure_file(
    "qc_performance_comparison.svg",
    "5-optimise_parameters/6-qc_performance_comparison"
  ), 
  "qc_performance_comparison", 
  "Comparison between method qc (x-axis) and method performance (y-axis)."
)
```
...
Based on the results of our benchmark, we created a set of practical guidelines for method users `r ref("fig", "user_guidelines")`. As method's performance is heavily dependent on the trajectory type being studied, the choice of method will by primarily driven by the prior knowledge of the user about what trajectory topology is expected in the data. For the majority of use cases, the user will know very little about the expected trajectory, except perhaps whether the data is expected to contain multiple trajectories, cycles or a complex tree structure. In each such use case, a different set of methods performed optimally, with Monocle DDRTree performing best when the data contains a complex tree, and Slingshot performing better when the trajectory could as well be less complex. For the use case where the data is expected to contain multiple trajectories or cycles, no methods were included in the current version of the evaluation, although AGA`r cite_methods("aga")` for disconnected trajectories, and Waddington-OT`r cite_methods("wadot")` and AGA`r cite_methods("aga")` for regular graphs are currently the only methods in literature able to handle these types of trajectories. If the user knows the exact topology, our evaluation suggests reCAT for cycles, SCORPIUS for linear trajectories, and Slingshot for bifurcating trajectories, although in the last case other topologies could also be returned by the method if it better fits the data. The most difficult use case is when the topology is known but more complex than a bifurcating or cyclic trajectory. Here, to our knowledge, only ElPiGraph.R [https://github.com/Albluca/ElPiGraph.R](github.com/Albluca/ElPiGraph.R), which is not yet published, can be used.


`r anchor("fig", "user_guidelines")`
![](`r figure_file("tree.svg", experiment_id="7-user_guidelines")`)
...
# Discussion
<!-- The context -->
Trajectory inference is unique among most other categories of single-cell analysis methods, such as clustering, normalisation and differential expression, because it models the data in a way that was almost impossible using bulk data. Indeed, for no other single-cell analysis types have so many tools been developed, according to several repositories such as omictools.org [@henry_omictoolsinformativedirectory_2014], the "awesome single cell software" list [@davis_awesomesinglecelllistsoftware_2018] and scRNA-tools.org [@zappia_exploringsinglecellrnaseq_2017] . It is therefore critical that these methods, now reaching `r sum(methods$is_ti, na.rm=TRUE)`, are evaluated to guide the users in their choice. In this preprint, we summarise the initial version of our evaluation of these methods, focusing on the quality control and the accuracy of their model using the default parameters on real and synthetic data.


<!-- The implications of the results -->
Our evaluation indicated a large difference between current TI methods, with Slingshot, followed by TSCAN and Monocle DDRTree, towering above all other methods. Nonetheless, we found that methods which did not perform well across all settings, can still be useful in certain specific use cases. Indeed, on data containing more than one bifurcations, Monocle DDRTree clearly performed better than other methods. We found that this was mainly because the default parameters of Monocle DDRTree prefer are more likely to detect tree topologies, while those of Slingshot prefer to find linear and bifurcation trajectories. 


<!-- Our common output model, advantages & limitations -->
We managed to wrap the output of all methods into one common format. This not only allowed us to compare different methods with a gold standard, but could also be useful for TI users because it allows the user to test multiple methods without manual conversion of input and output. Furthermore, it makes it possible to compare the output of different methods, which opens up possibilities for new comparative visualisation techniques or ensemble methods. However, we acknowledge that our model has some limitations. It can currently not take into account uncertainty of the cell's position. This can be both locally, for example uncertainty of its position within one branch, or globally, for example uncertainty over connections between branches. Some current methods already model this uncertainty in some way, mainly locally `r cite_methods("ouija")`, and in the future we will adapt our output model to also allow this uncertainty.


<!-- Synthetic data: observations and possibilities -->
The use of synthetic data to evaluate methods can be problematic, because the model to generate the data is not necessarily similar to the way a true biological system works. This could bias an evaluation towards methods for which the underlying model best fits the model used for generating data. Therefore, it is essential that results of an evaluation on synthetic data are confirmed using real data. In our study, the overall performance of methods was very similar between real and synthetic datasets, confirming the biological relevance of the synthetic data. Given this, we believe our synthetic data can now be used to effectively prototype new TI methods, which use ever more complex models, such as disconnected graphs. Furthermore, it is expected that in the future methods will be able to model even more complex behavior, such as multiple dynamic processes happening at parallel in a single cell, the integration of datasets from different patients or trajectories in a spatial context [@tanay_scaling_2017; @cannoodt_computational_2016]. Synthetic data based on our workflow could therefore be used to spearhead the development of these methods, given that currently only a limited number of real datasets are available for which the methods could be useful. Furthermore, we believe our evaluation workflow could also be used to evaluate other types of single-cell modelling techniques, such as single-cell network inference, clustering and normalisation. We sincerely hope that such efforts will lead to a more rapid development of accurate methods, and will in the near future provide a package which can be used to simulate synthetic data for a wide variety of single-cell modelling problems.
...
### Method selection
We gathered a list of `r sum(methods$is_ti, na.rm=TRUE)` trajectory inference methods (`r ref("table", "implementations")`, als), by searching in literature for "trajectory inference" and "pseudotemporal ordering", and based on two existing lists found online [@davis_awesomesinglecelllistsoftware_2018; @gitter_singlecellpseudotimeoverviewalgorithms_2018]. We welcome any contributions by creating an issue at [https://github.com/dynverse/dynalysis/issues](github.com/dynverse/dynverse/issues). Methods were excluded from the evaluation based on several criteria: `r glue::collapse(glue::glue("({non_inclusion_reasons$footnote}) {non_inclusion_reasons$long}"), ", ")`. This resulted in the inclusion of `r sum(methods$evaluated)` methods in the evaluation (`r ref("table", "implementations")`).
...
* **Grouping** For each cell a label to which state/cluster/branch it belongs. For real data, the states from the gold/silver standard. For synthetic data, each milestone was seen as one group, and cells were assigned to its closed milestone.
...
```{r}
add_sfig(
  figure_file('wrapping.svg', '4-method_characterisation'),
  "wrapping", 
...
## Real datasets
We gathered a list of real datasets by searching for "single-cell" at the Gene Expression Omnibus and selecting those datasets in which the cells are sampled from different stages in a dynamic process (`r ref("stable", "datasets")` and `r ref("sfig", "dataset_characterisation")`). The scripts to download and process these datasets will be made available n our repository ([http://www.github.com/dynverse/dynalysis](github.com/dynverse/dynalysis)). When available, we preferred to start from the raw counts data. These raw counts were all normalised and filtered using a common pipeline, discussed later. We determined a reference standard for every dataset using labelling provided by the author's, and classified the standards into gold and silver based on whether this labelling was determined by the expert using the expression (silver standard) or using other external information (such as FACS or the origin of the sample, gold standard) (`r ref("stable", "datasets")`).
...
Our workflow to generate synthetic data is based on the well established workflow used in the evaluation of network inference methods [@schaffter_genenetweaversilicobenchmark_2011; @marbach_wisdom_2012] and consists of four main steps: network generation, simulation, gold standard extraction and simulation of the scRNA-seq experiment (`r ref("sfig", "synthetic_data_generation")`). At every step, we took great care was taking to mimic real cellular regulatory networks as best as possible, while keeping the model simple and easily extendable. For every synthetic dataset, we used a random real dataset as a reference dataset (from those described earlier), making sure the number of variable genes and cells were similar.
...
add_sfig(
  read_rds(figure_file(
    "modulenets.rds",
     experiment_id = "2-dataset_characterisation/3-synthetic"
   )), 
  "modulenets", 
...
## Evaluation metrics
We used three different metrics to assess the performance of each TI method (`r ref("fig", "evaluation_overview")`). The importance of using multiple metrics to compare complex models has been stated repeatedly [@norel_selfassessmenttrap_2011]. Indeed, each of our metrics looks at the correspondence between prediction and gold standard in a different way: the correlation between geodesic distances assesses the overall ordering, the RF prediction error assesses the cell neighbourhood, and the edge flip score assesses the milestone network topology (`r ref("sfig", "score_aspects")`).
...
add_sfig({
  read_rds(figure_file(
    "score_aspects.rds", 
    experiment_id="3-score_rules/5-score_aspects"))
  }, 
  "score_aspects", 
...
```{r}
add_sfig(
  figure_file("metric_aggregation_overview_v1.svg", experiment_id="evaluation_overview"), 
...
### Pipeline
<!-- Look at dyneval methods to write this paragraph -->



### Method execution
We ran each method in a controlled environment, where we limited the memory usage (max. 32GB) and total run time (max 24h). Moreover, we disabled the function of the R `set.seed` function, to avoid that methods false make their output deterministic. 
...
We classified all possible trajectory topologies into distinct trajectory types, based on topological criteria. These trajectory types start from the most general trajectory type, a disconnected directed graph, and moves down (within a directed acyclic graph structure), progressively becoming more simple until the two basic types: linear and cycles (`r ref("sfig", "trajectory_type_tree", "a")`). For every directed trajectory type, a corresponding undirected trajectory type can also be defined (`r ref("sfig", "trajectory_type_tree", "b")`). In most cases, a method which can detect a complex trajectory type, was also be able to detect less complex trajectory types, with the only exception being DPT (limited to bifurcating trajectories). 


```{r}
add_sfig(
  figure_file("trajectory_type_trees.svg","2-dataset_characterisation/2-trajectory_types"), 
  "trajectory_type_tree", 
  "Different trajectory types can be classified in a directed acyclic graph structure, in which certain types are a generalisation of other trajectory types.", 
  15, 
  10
)
```
...
git2r::repository(".")
```
This paragraph may be expanded
Done (stolen from results section)
🤔
Add the timecourse prior to the figure
LINK TO 
...
### Trajectory types and topology complexity
It's already a bit better, though could be improved still!
Expand, but maybe it should be moved to discussion anyway
ADD CAPTION
😉
TODO
ADD CAPTION
FILL IN SIGNIFICANCE LEVELS
to do: add metric figure
needs to be updated with waypoints
this is not used anymore
TODO
More to add here?
* Look at dynmethods::execute_method to write this paragraph
Are there more exectuion details?