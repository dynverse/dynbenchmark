...
```{r}
flin <- implementations %>%
filter(fixes_topology != 'free', maximal_trajectory_type == 'directed_linear') %>% .$implementation_id
fbif <- implementations %>%
filter(fixes_topology != 'free', maximal_trajectory_type == 'bifurcation') %>% .$implementation_id
foth <- implementations %>% 
 filter(fixes_topology != 'free', !maximal_trajectory_type %in% c('directed_linear', 'bifurcation')) %>%
 .$implementation_id
falg <- implementations %>% filter(fixes_topology == 'algorithm') %>% .$implementation_id
fpar <- implementations %>% filter(fixes_topology == 'parameter') %>% .$implementation_id
...
A plethora of TI methods has been developed over the last years, and more are being published at a rate of about one method every two weeks (`r ref("sfig", "methods_per_year", "a")`).
Surprisingly, almost all methods have a unique combination of key characteristics in terms of the required inputs, produced outputs and methodology used.
<!-- §3: TOPOLOGY FIXING VERSUS TOPOLOGY INFERENCE -->
Initially, most trajectory inference methods fixed the trajectory topology to a linear `r cite_methods(flin)`, bifurcating `r cite_methods(fbif)` or other `r cite_methods(foth)` topology, by either algorithmic limitations `r cite_methods(falg)` or through user parameters `r cite_methods(fpar)`. The focus of these methods was therefore mainly to correctly order the cells along this fixed topology. However, several recent methods now infer both the ordering and the topology at the same time, although some topological limitations are still imposed. Methods which do not impose limitations on the topology at all are now being developed [@welch_slicerinferringbranched_2016; @wolf_graphabstractionreconciles_2017], and it is expected that in the future methods will be able to model even more complex behavior, such as multiple dynamic processes happening at parallel in a single cell or the integration of datasets from different patients [@tanay_scaling_2017; @cannoodt_computational_2016].


<!-- §4: STEPS AND COMPONENTS -->
Although trajectory inference methods use a variety of algorithms and steps to reach a final ordering, most consist of the following three main steps: (i) preprocessing (normalization, filtering of genes and/or cells), (ii) conversion to a simplified representation using dimensionality reduction, clustering or graph building and (iii) ordering the cells along the simplified representation [@cannoodt_computational_2016]. 


<!-- §5: OPTIONAL OR REQUIRED PRIORS -->
...
We gathered a list of `r sum(methods$is_ti, na.rm=TRUE)` TI methods  (`r ref("fig", "evaluation_overview", "b")`), and selected a subset of `r sum(methods$evaluated)` for evaluation, primarily based on their free availability, the presence of a programming interface, and the time they were published (`r ref("table", "implementations")`). Currently, only methods published before june 2017 are included in the current evaluation, while more recent methods will be included in the next version of the evaluation. We characterised and benchmarked each method in six different ways: its detectable trajectory topology, its performance on real and synthetic datasets, its running time and scalability, its requirements for prior information, the underlying algorithm, and the quality of the tools. 


The main distinguishable feature among TI methods is the extent to which a method can adapt its trajectory topology to a dataset, with … of the `r sum(methods$evaluated)` evaluated methods fixing the trajectory type either due to algorithmic limitations or parameters. Moreover, if a method's topology is free, the topology can still be constrained, limiting the number of trajectory types a method can detect. Although a significant number of methods can only predict linear or cyclical trajectories, a majority of methods can handle one or more splits (either bifurcations or multifurcations) in the trajectory (`r ref("fig", "implementation_overview", "a")`). However, only a handful, mostly recent, methods can handle more complex graph topologies which can include loops (`r ref("sfig", "methods_over_time", "b")`), one of which is included in the current evaluation (SLICER).
...
Except for multifurcations, acyclic graphs and graphs, the top performing method was also a method which can handle that particular trajectory type in theory (`r ref("sfig", "trajtype_handle_comparison")`). This higher performance was mainly due to a higher edge flip and random forest MSE score, as the correlation score was in almost every case (except for linear trajectories) higher for methods which cannot handle the particular topology type. This indicates that among the current methods, one has to balance finding the correct topology against optimally ordering the cells along this topology. For acyclic graphs and graphs, only one method was included in the study (SLICER), and it did not perform well on any trajectory type. For multifurcations, wishbone slightly outperformed slingshot and other methods able to detect multifurcations, …….. Ugh this is difficult to explain :p
...
Despite their similar overall performance, the topologies predicted by the top methods differed considerably. Trajectories detected by the default parameters of slingshot and cellTree tended to be simpler of structure, while those detected by TSCAN and Monocle DDRTree gravitated towards more complex topologies (`r ref("fig", "topology_complexity_comparison")`). Monocle DDRTree for example, frequently predicted a tree topology, despite only a cyclic, linear or bifurcating topology being present in the data (`r ref("fig", "topology_complexity_comparison", "a")`). Trajectories generated by Monocle DDRTree (at default parameters) tend to contain more nodes and edges (`r ref("fig", "topology_complexity_comparison", "b")`), which could give this method an edge on datasets with complex tree topologies, but could also explain its relatively low performance on linear and bifurcating datasets. Indeed, when we assessed how often a method can predict the correct topology, slingshot and TSCAN were good at correctly predicting linear, bifurcating and converging trajectories, while Monocle DDRTree was the only method correctly predicting tree topologies. Nonetheless, the overall accuracy of topology prediction was very low, with Slingshot correctly predicting bifurcating and converging topologies for half of the datasets, and Monocle DDRTree predicting the correct tree topology in 12% of the cases (`r ref("fig", "topology_sensitivity")`). Therefore, current methods still require a considerable guidance by the end user, to either optimise the parameters or to choose the method which best fits the user's expert knowledge.
...
I'm still on the fence. I think we don't even discuss TI in enough detail, let alone transcriptomics vs. cytometry
But TI methods are also central in cytometry
is this the best place for this citation?
TODO: explicitly show the number of TI methods each year, to support our claim of '1 method every 2 weeks'.


For example, see:
figure_file('ti_methods_per_year.svg', '4-method_characterisation')
Use different word since it was already used in §2
