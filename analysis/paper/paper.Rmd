...
Using single-cell transcriptomics or proteomics data, it is now possible to computationally order cells along trajectories, allowing the unbiased transcriptome-wide study of cellular dynamic processes. Since 2014, more than 50 trajectory inference methods have been developed, each with its own set of methodological characteristics. As a result, choosing a method to infer trajectories is often challenging, since a comprehensive assessment of the performance and robustness of each method is still lacking. In order to facilitate the comparison of the results of these methods to each other and to a gold standard, we developed a global framework to benchmark trajectory inference tools. . Using this framework, we compared the results from a total of  `r sum(methods$evaluated)` trajectory inference methods, on a large collection of real and synthetic datasets. We evaluate methods using several metrics, including accuracy of the inferred ordering, correctness of the network topology, code quality and user friendliness. 
...
add_table(
  ...
  "Overview of the trajectory inference methods included in this study, and several characteristics thereof. This table will be continuously updated at https://goo.gl/FnsTxG."
)
```
...
Having ordered all methods by their overall benchmarking score, we found that Slingshot predicted the most accurate trajectories, followed by TSCAN and Monocle DDRTree. When we looked at the benchmark scores per trajectory type, Slingshot was to be the only method that performs well across most trajectory types. However, we found that several methods were specialised in predicting specific trajectory types; for example SCORPIUS for linear trajectories, reCAT for cycles, Monocle DDRTree for rooted trees, and Wishbone for multifurcations. This last result is unexpected, as Wishbone is not able to output any multifurcating topologies. We will discuss the effect of topology types and constraints on inferrable trajectory types placed by the method in Section SEC_TRAJTYPE. 
...
* An evaluation of the stability of method's results when running the same parameter setting on the same dataset multiple times
...
Depending on the output of a method, we used different strategies to convert the output to our model (`r ref("sfig", "wrapping", "b")`). Special conversions are denoted by an \*.
...
* **Type 1, direct:** Methods which assign each cell to a branch together with a pseudotime value and a branch network: `r conversion("branching_local")`. The branch network is used as the milestone network, the percentages of a cell are proportional with its branch pseudotime.
* **Type 2, linear pseudotime:** Methods returning a single pseudotime value: `r conversion("linear")`. A single edge between two milestones, its percentage proportional to the pseudotime.
* **Type 3, end state probability:** Methods returning both a global pseudotime value and a probability for every end state: `r conversion("branching_global")`. We use a single start state and add an edge to every end milestone each representing an end state. The global pseudotime then determines the distance from the begin milestone, the rest of the cell's position is calculated by distributing the residual percentage over the end states, proportionally to the end state probabilities.
* **Type 4, cluster assignment:** Methods returning a cluster assignment and a cluster network:  `r conversion("branching_cluster")`. The cluster network was used as milestone network, each cell received percentage 1 or 0 based on its cluster assignment.
* **Type 5, projection onto nearest branch:** Methods returning a cluster assignment, cluster network and dimensionality reduction: `r conversion("branching_local_projection")`. We projected each cell on the closest point on the edges between its own cluster and neighbouring clusters within the dimensionality reduction. This projection allowed us to give each cell a pseudotime within the edge, which was then converted into our model as described above, using the cluster network as milestone network.
...
## Real datasets
We gathered a list of real datasets by searching for "single-cell" at the Gene Expression Omnibus and selecting those datasets in which the cells are sampled from different stages in a dynamic process (`r ref("stable", "datasets")` and `r ref("sfig", "dataset_characterisation")`). The scripts to download and process these datasets will be made available n our repository ([github.com/dynverse/dynalysis](http://www.github.com/dynverse/dynalysis)). When available, we preferred to start from the raw counts data. These raw counts were all normalised and filtered using a common pipeline, discussed later. We determined a reference standard for every dataset using labelling provided by the author's, and classified the standards into gold and silver based on whether this labelling was determined by the expert using the expression (silver standard) or using other external information (such as FACS or the origin of the sample, gold standard) (`r ref("stable", "datasets")`).
...
git2r::repository(".")
```
I would avoid making "absolute" statements like this one.
Add the timecourse prior to the figure
Changed the tenses here to past (results usually only include past tense, except when referring to figures or later in the text)
why surprisingly?
because wishbone is only able to detect bifurcations. Should mention this in the text :)
Added an explanation in the following sentence
But still more should be added, I think this simply indicates that none of the methods are performing well on multifurcating datasets. But we should show this, eg. looking at the raw performance on multifurcating datasets versus other datasets. If we don't have the time to show this, then we should not mention wishbone here
LINK TO 
