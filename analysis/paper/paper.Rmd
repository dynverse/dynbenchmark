...
* Inclusion of more recent methods
...
* **Type 2, linear pseudotime:** `r conversion("linear")`. These methods return a single pseudotime value. A single edge between two milestones, its percentage proportional to the pseudotime.
...
* **Type 6, cell graph:** `r conversion("cell_graph")`. These methods return a network of connected cells, and determine which cells are part of the "backbone". Cell
...
maybe add something like "which we believe to be the largest list of TI methods to be compared in a single study"?
...
do you talk about the synthetic data or the workflow? Because I'm not sure if the wrappers, comparison metrics.. could be used as they are to compare NI, clustering or normalisation methods. Or if you are convinced of this statement, then I would add one or two sentences to explain how it could be possible.
