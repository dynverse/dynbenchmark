...
```{r}
fixed_linear <- methods %>%
 filter(
   fixes_topology != 'free', 
   maximal_trajectory_type != 'undirected_linear'
 ) %>%
 .$method_id
fixed_bifurcating <- methods %>%
 filter(
   fixes_topology != 'free', 
   maximal_trajectory_type != 'simple_fork'
 ) %>%
 .$method_id
fixed_other <- methods %>%
 filter(
   fixes_topology != 'free', 
   !maximal_trajectory_type %in% c('undirected_linear', 'simple_fork')
 ) %>%
 .$method_id
fixed_algorithm <- methods %>%
 filter(fixes_topology == 'algorithm') %>%
 .$method_id
fixed_parameters <- methods %>%
...
git2r::repository(".")
```
When sampled, single cells will most probably be in different states of dynamic processes, due to the stochasticity of expression. Trajectory inference methods can be used to computationally order these cells along trajectories, based on gradual changes in the expression of several genes.
It is not really due to stochasticity that these cells are in different states of a dynamic process though?
insert refs
get an idea of which available method or set of methods would optimally resolve their problem.
we present the first comprehensive (...)?
compared 22 TI methods on both synthetic (..)
