...
Using single-cell transcriptomics data, it is now possible to computationally order cells along trajectories, allowing the unbiased transcriptome-wide study of cellular dynamic processes. Since 2014, more than 50 trajectory inference methods have been developed, each with its own set of methodological characteristics. This makes a comparison between trajectory inference methods often difficult, and as a result a comprehensive assessment of the performance and robustness of each method is still lacking. We developed a common data structure for trajectory inference methods in order to make their results comparable to each other and to a gold standard.[a] Using this framework, we compare the results from a total of 22 trajectory inference methods, on a large collection of real datasets and realistic synthetic datasets. We employ several metrics, including accuracy of the inferred ordering, correctness of the network topology, code quality and user friendliness. We conclude with practical guidelines for method[b] users, and also believe our evaluation framework can be used to spearhead the development for new methods. Our evaluation pipeline is fully reproducible, can be easily extended, and is available at https://www.github.com/dynverse.
...
Single-cell -omics technologies now make it possible to more accurately[c] model biological systems than ever before. One area where single-cell data can be particularly useful is in the study of cellular dynamic processes, such as the cell cycle, cell differentiation and cell activation. When cells are sampled from a population in which cells are at different unknown points in the dynamic process,[d] trajectory inference methods can be used to computationally order these cells along their dynamic process [@cannoodt_computational_2016]. Because they offer an unbiased and transcriptome-wide understanding of the dynamic process[e], trajectories then allow the identification of new [f](primed) subsets of cells, delineation the exact wiring of a differentiation tree and inference of regulatory interaction responsible for a bifurcation refs. This both for specific subsets of cells, but in the near future also for whole tissues or organisms[g] [@regev_scienceforumhuman_2017].
...
```{r}
fixed_linear <-methods$method_id[which(methods$fixes_topology != 'free' & methods$maximal_trajectory_type == 'undirected_linear')]
fixed_bifurcating <-methods$method_id[which(methods$fixes_topology != 'free' & methods$maximal_trajectory_type == 'simple_fork')]
fixed_other <-methods$method_id[which(methods$fixes_topology != 'free' & methods$maximal_trajectory_type != 'undirected_linear'& methods$maximal_trajectory_type != 'simple_fork')]
fixed_algorithm <-methods$method_id[which(methods$fixes_topology == 'algorithm')]
fixed_parameters <-methods$method_id[which(methods$fixes_topology == 'parameter')]
```


Dozens of trajectory inference methods have been developed over the last years, and more are being published almost every month (`r ref("sfig", "methods_over_time", "a")`). Initially, most trajectory inference methods fixed the trajectory topology to a linear `r cite_methods(fixed_linear)`, bifurcating `r cite_methods(fixed_bifurcating)` or other `r cite_methods(fixed_other)` topology, by either algorithmic limitations `r cite_methods(fixed_algorithm)` or through user parameters `r cite_methods(fixed_parameters)`. The focus of these methods was therefore mainly to correctly ordering[h] the cells along this[i] fixed topology. However, several recent methods now infer both the ordering and the topology at the same time, although some topological limitations are still imposed (`r ref("sfig", "methods_over_time", "b")`). Methods which do not impose limitations on the topology at all are now being developed [@welch_slicer:_2016; @wolf_graph_2017], and it is expected that in the future methods will be able to[j] model even more complex behavior, such as multiple dynamic processes happening at parallel in a single-cell or the integration of datasets from different patients [@tanay_scaling_2017; @cannoodt_computational_2016].


<!-- Common framework of TI, different components, naturally progressing towards the need for an evaluation -->
Although trajectory inference methods use a variety of algorithms and steps to reach a final ordering, most consist of the following three main steps: (i) preprocessing (normalization, filtering of genes and/or cells), (ii) conversion to a simplified representation using dimensionality reduction, clustering or graph building and (iii) ordering the cells along the simplified representation [@cannoodt_computational_2016]. Depending on the way the cells are ordered, some prior information about the dynamic process can be optionally used or required, which can both bias the trajectory to current knowledge but also guide the method towards choosing the right trajectory among many others. Furthermore, with the ongoing scalability in single-cell -omics technologies (reaching a million cells [@angerer_singlecellsmake_2017], it becomes more and more important that analysis methods become both scalable [@aibar_scenicsinglecellregulatory_2017; @angerer_singlecellsmake_2017] and user friendly.


<!-- Common framework of TI, different components, naturally progressing towards the need for an evaluation -->
Given this plethora of available trajectory inference methods, it is important that methods are compared, so that users can use the most optimal method available for their problem[k]. Moreover, new methods need to be rigorously tested, so that development can focus on improving the current state-of-the-art. In this study, we therefore for the first time developed a[l] comprehensive evaluation framework for trajectory inference methods. We test[m] both on synthetic data, for which the gold standard is known, and real data, for which in some cases the gold standard can be extracted from expert knowledge. We include a rigorous parameter optimisation, making sure that parameters are optimized while avoiding overfitting on characteristics of specific datasets. To make our framework reusable and extendable, we make use of continuous analysis [@beaulieu-jones_reproducibility_2017], which allows the developers of new methods to easily test their methods and compare them against the state-of-the-art.
...
At the level of trajectory topologies, most recent methods can handle one or more splits (either bifurcations or multifurcations) in the trajectory (`r ref("fig", "method_characterisation", "a")`). However, only a handful, mostly recent[n], methods can handle more complex graph topologies which can include loops (`r ref("sfig", "methods_over_time", "b")`), one of which is included in the current evaluation (SLICER).
...
`r anchor("fig", "method_characterisation")`
![](`r figure_file("method_characteristics.svg", experiment_id="4-method_characterisation")`)
`r ref("fig", "method_characterisation", anchor=TRUE)` First characterization of each method.


```{r}
add_sfig({
read_rds(figure_file("implementations_table.rds", experiment_id="4-method_characterisation"))
}, 
"implementations", 
"Caption", 
10, 
...
Most[o] methods fulfill most of the basic criteria, such as free availability and basic code quality criteria (`r ref("fig", "method_characterisation", "b")`). Recent methods tend to have higher code quality (`r ref("sfig", "qc_over_time")`). However, several aspects are consistently lacking for the majority of the methods (`r ref("fig", "method_characterisation", "e")`) and we believe that these should receive extra attention from developers. Although these outstanding issues cover all five categories, code assurance and documentation in particular are problematic areas (`r ref("sfig", "implementation_qc_check")` and `r ref("fig", "method_characterisation", "c")`), notwithstanding several studies pinpointing these as good practices [@wilson_best_2014; @artaza_top10metrics_2016].
...
```{r}
add_sfig({
                glue::glue("![]({figure_file('wrapping.svg', experiment_id = '4-method_characterisation')})")
}, 
"wrapping", 
...
conversion <- function(conversion_oi) {
    methods_evaluated %>% filter(map_lgl(conversion_split, ~ conversion_oi %in% .)) %>% mutate(text = paste0(method_name, ifelse(conversion_special, "\\*", ""))) %>% pull(text) %>% list_sentence()
}
```
...
git2r::repository(".")
```
[a]which facilitates the comparison of the results of trajectory inference methods (start using the TI abbreviation at some point?) between each other, and possibly to a gold standard.
[b]which might come in handy to users. We believe (...)
[c]move? "make it possible to model biological systems more accurately than ever before".
[d]When sampled, single cells will most probably be in different states of dynamic processes, due to the stochasticity of expression. Trajectory inference methods can be used to computationally order these cells along trajectories, based on gradual changes in the expression of several genes.
[e]processes?
[f]the trajectories can allow to identify new (primed) subsets of cells, to delineate the exact wiring of a differentiation tree and to infer the regulatory interactions which are responsible for a bifurcation.
[g]This is currently applicable to specific subsets of cells, but might also be applicable to whole tissues or organisms in the near future.
[h]order
[i]a?
[j]will be able to model any underlying structure in the data, without the need of user provided parameters. These methods might allow to model more complex behaviors, such as (...)
[k]get an idea of which available method or set of methods would optimally resolve their problem.
[l]we present the first comprehensive (...)?
[m]compared 22 TI methods on both synthetic (..)
[n]a handful of the most recent methods
[o]Repetition of most: put "The majority of" instead?