...
Overall, we find that slingshot predicts the most accurate trajectories across different data sources, trajectory types and metrics, followed by TSCAN and Monocle DDRTree CREF. These results are consistent between real and synthetic datasets CREF, confirming both the relevance of the synthetic data and the accuracy of the real datasets' gold and silver standards. However, results are more variable when comparing the three different evaluation metrics, 
## 


The main distinguishable feature among TI methods is the extent to which a method can adapt its trajectory topology to a dataset. In some 
## Prior information
...
We characterised all methods in four different ways: possible trajectory topologies, implementation quality, prior information and the underlying algorithm (`r ref("fig", "implementation_overview")`).
...
![](`r figure_file("method_components.svg", experiment_id="4-method_characterisation")`)
...
Performance was stable between real and synthetic datasets `r ref("sfig", "real_synthetic_comparison")`
...
"Comparison of method performance between real and synthetic datasets. ", 
12, 
4
)
```
