...
Recent technological advances allow unbiased investigation of cellular dynamic processes, by generating -omics profiles of thousands of single cells and computationally ordering these cells along a trajectory. Since 2014, at least `r sum(implementations$contains_ti, na.rm=TRUE)` so-called trajectory inference methods have been developed, but due to high variability in their inputs and outputs, these methods are difficult to compare. As a result, a comprehensive assessment of the performance and robustness of trajectory inference methods is still lacking, and more importantly also guidelines for new users to the field. 


In this study, we evaluated a total of  `r sum(methods$evaluated)` methods in terms of  cellular ordering, trajectory topology, code quality and user friendliness. Our results indicate that slingshot is the most accurate method to date, but other leading methods (e.g. TSCAN, Monocle DDRTree, SCORPIUS and cellTree) may perform better in specific use-cases. We therefore developed a set of guidelines to aid users select the most appropriate method. While the field certainly has matured significantly since its inception, further progress is still necessary in order to cope with ever-more complex use cases. We hope to spearhead such developments and the field as a whole, by making our evaluation pipeline easily extendable and freely available at [github.com/dynverse/dynverse](https://www.github.com/dynverse/dynverse).



# Introduction
Single-cell -omics technologies now make it possible to model biological systems more accurately than ever before [@tanay_scaling_2017]. One area where single-cell data has been particularly useful is in the study of cellular dynamic processes, such as the cell cycle, cell differentiation and cell activation [@etzrodt_quantitativesinglecellapproaches_2014]. Such dynamic processes can be modelled computationally using using trajectory inference (TI), which order cells along a trajectory by their overall similarity in -omics datasets (mainly transcriptomics) [@trapnell_definingcelltypes_2015; @cannoodt_computational_2016; @moon_manifoldlearningbasedmethods_2018]. The resulting trajectories are most often linear, bifurcating or tree-shaped, but more recent methods also allow for more complex trajectory topologies such as cyclic or disconnected graphs. TI methods offer an unbiased and transcriptome-wide understanding of a dynamic process [@tanay_scaling_2017],  thereby allowing the objective identification of new (primed) subsets of cells [@schlitzer_identificationcdc1cdc2committed_2015], delineation of a differentiation tree [@velten_humanhaematopoieticstem_2017; @see_mappinghumandc_2017] and inference of regulatory interactions responsible for one or more bifurcations [@aibar_scenicsinglecellregulatory_2017]. 
...
In this study, we performed a comprehensive evaluation of  `r sum(methods$evaluated)` TI methods (`r ref("fig", "evaluation_overview", "a")`). The inclusion criterion for TI methods was primarily based on their free availability and presence of a programming interface (`r ref("table", "implementations")`). The evaluation comprised three core aspects: (i) assessment of the accuracy and scalability of TI methods by comparing predicted trajectories with a gold standard on more than 200 real and synthetic datasets, (ii) a quality control of the provided software and documentation, and (iii) a combination of both the performance and QC evaluations to create a set of practical guidelines for users.
...
Across all datasets and evaluation metrics, we found that slingshot, TSCAN, Scorpius and CellTree predicted the most accurate trajectories (`r ref("fig", "results_overview", "b")`). When we focused on the benchmark scores per trajectory type, we found that several methods were specialised in predicting specific trajectory types; for example SCORPIUS returned the most accurate linear trajectories, reCAT optimally identifies cycles, and Monocle DDRTree most accurately identifies trees. This was also reflected in the rankings of the different metrics, with methods able to detect more complex trajectories, such as DPT, slingshot, TSCAN and Monocle DDRTree, obtaining a better topological similarity score, while other methods, including SCORPIUS, slingshot and cellTree, were better at predicting the ordering and neighbourhood of the cells (`r ref("fig", "results_overview", "b")`).
...
<!-- Errors-->
Several methods obtained a high overall score despite having 5 to 10% of failed executions (e.g. Monocle DDRTree, cellTree Gibbs, SLICE), meaning these methods could rank even higher if not for the failed executions. Methods that do not scale well with respect to the number of cells or genes exceeded the pre-defined time or memory limits for the largest datasets (reCAT, SCOUP, StemID, MFA, cellTree Gibbs). For a few methods, the time or memory limits were exceeded too often, making the benchmarking results difficult to compare to those of other methods (Ouija, Pseudogp).
...
Apart from the trajectory types, we also investigated several other factors which could influence the performance of a method. Although we provided prior information to those methods which require it, we did not observe any major difference in method performance between these methods and methods that did not receive any prior information (`r ref("fig", "results_overview", "b")`). When we looked at the algorithmic steps shared by different methods, we found that those which included principal curves (such as slingshot and SCORPIUS), k-means (which include slingshot and several other top scoring methods) and some graph building (which include almost every top scoring method) tended to have a slightly higher performance ( `r ref("stable", "component_importance")`). On the other hand, methods using t-SNE and ICA for their dimensionality reduction were ranked lower, although this was not statistically significant.
...
<!-- The context -->
Trajectory inference is unique among most other categories of single-cell analysis methods, such as clustering, normalisation and differential expression, because it models the data in a way that was almost impossible using bulk data. Indeed, for no other single-cell analysis types have so many tools been developed, according to several repositories such as omictools.org [@henry_omictoolsinformativedirectory_2014], the "awesome single cell software" list [@davis_awesomesinglecelllistsoftware_2018] and scRNA-tools.org [@zappia_exploringsinglecellrnaseq_2017]. It is therefore critical that these methods, now reaching `r sum(implementations$contains_ti, na.rm=TRUE)`, are evaluated to guide users in their choice. In this study, we presented a large scale evaluation of the performance and code of  `r sum(methods$evaluated)` TI methods. When comparing the maximum overall score over time, it is encouraging to see multiple incremental improvements over the state-of-the-art. We believe that the benchmarking presented in this study will pave the way to the next series improvements in the field of trajectory inference.




In this study, we presented a large scale evaluation of the performance and code of  `r sum(methods$evaluated)` TI methods. In order to make the outputs of the TI methods more comparable, we developed a new common format. This not only allowed us to assess the performance of the methods compared to a gold standard, but may also facilitate the use of these methods by TI users, as multiple methods could be run on the same dataset and their results could be compared without manual conversion of input and output. Furthermore, the common structure opens up possibilities for new comparative visualisation techniques or ensemble methods. However, we acknowledge that our model has some limitations. Currently, It cannot take into account uncertainty of a cell's position, which can both occur on the cellular ordering (e.g. when the position of a cell is uncertain within a branch)  or on the trajectory topology (e.g. when the connections between branches are uncertain). Some current methods already model this uncertainty in some way, mainly on the cellular ordering `r cite_methods("ouija")`, and in the future we will adapt our output model to also allow this uncertainty. 


Although this study included the comparison of more TI methods than ever before (cite other methods which compared themselves to 3-4 others), the field keeps evolving, requiring a constant upgrade of the benchmarking. We believe that the common structure for TI results and the comparative metrics that we generated for this study, as well as the high number of datasets that we both gathered from the literature and generated synthetically represent a solid potential for benchmarking new methods. We will make the datasets available on “github..” and the tools necessary for the comparison of methods available on “github.. Dyneval”, therefore providing the material for authors of new TI methods to robustly assess the performance of their method.


Discuss advantages of synthetic data and how it could be used for other studies?


Our evaluation indicated a large heterogeneity in the performance of the current TI methods, with Slingshot, TSCAN, Monocle DDRTree, SCORPIUS and cellTree towering above all other methods. Nonetheless, we found that methods which did not perform well across all settings, such as reCAT, could still be highly performant in certain specific use cases. By comparing TI methods on a large set of datasets, we were able to extract characteristics concerning which methods performed best in which case. Currently, all TI methods were compared by using the default parameters set in their implementation. We decided to use this setting for two reasons. 1) the setting of several parameters requires a certain knowledge of the data structure, which is not always known of TI users and 2) as certain methods require more parameters than others, we meant to prevent these methods from being over-ranked. Nonetheless, we plan to apply parameter tuning to all the methods in the near future, in order to benchmark the TI methods’ behaviour with optimal parameters.


Discuss code quality, how it tends to improve with time, and how we believe good code quality leads to “better science”?
...
### Method selection
We gathered a list of `r sum(implementations$contains_ti, na.rm=TRUE)` trajectory inference methods (`r ref("table", "implementations")`), by searching in literature for "trajectory inference" and "pseudotemporal ordering", and based on two existing lists found online [@davis_awesomesinglecelllistsoftware_2018; @gitter_singlecellpseudotimeoverviewalgorithms_2018].
...
<!-- PASTED FROM INTRODUCTION, need to rework -->
<!-- §4: Prior information -->
...
Our workflow to generate synthetic data is based on the well established workflow used in the evaluation of network inference methods [@schaffter_genenetweaversilicobenchmark_2011; @marbach_wisdom_2012] and consists of four main steps: network generation, simulation, gold standard extraction and simulation of the scRNA-seq experiment. At every step, we took great care to mimic real cellular regulatory networks as best as possible, while keeping the model simple and easily extendable. For every synthetic dataset, we used a random real dataset as a reference dataset (from those described earlier), making sure the number of variable genes and cells were similar.
...
```{r}
add_sfig(
  figure_file("score_aspects_v2.svg", experiment_id="3-score_rules/5-score_aspects"), 
  "score_aspects", 
...
```{r}
add_sfig(
  figure_file("metric_geodesic.svg", experiment_id="evaluation_overview"), 
  "metric_geodesic", 
...
add_sfig(
  ...
  "**Comparative analysis between various facets of the results.** a) Comparison of method performance between real and synthetic datasets across trajectory types. Given in the top-left corner of each panel is the Pearson's correlation. b) Violin plots of the overall performance. For each method, the arch indicates next method in the ranking over which one has significant improvement in performance over the other (top; pairwise wilcoxon test corrected for multiple testing; p-value < 0.05)."
)
```
...
add_stable(
  ...
  "Overview of the trajectory inference methods included in this study, and several characteristics thereof."
)
```
