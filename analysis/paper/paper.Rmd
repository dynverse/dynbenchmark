...
ref <- function(ref_type, ref_id, suffix="", anchor=FALSE) {
  ...
  ref <- refs %>% filter(ref_id == !!ref_id) %>% 
    pull(name) %>% 
    {glue::glue("[**{.}{suffix}**](#{ref_type}_{ref_id})")}
  
  if (anchor) ref <- glue::glue("{ref}<a name='{ref_type}_{ref_id}'></a>")
  ref
}
create_names <- list(
...
method_characteristics <- readRDS(figure_file("method_characteristics.rds", experiment_name="method_characteristics"))
...
`r ref("fig", "method_characteristics", anchor=TRUE)` Evaluation overview and method overview.
