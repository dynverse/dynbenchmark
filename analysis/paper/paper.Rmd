...
Finally, using both the benchmark results and quality control, we produced a flow chart with practical guidelines for selecting the most appropriate TI method for a given use case.
...
Across all datasets and evaluation metrics, we found that Slingshot predicted the most accurate trajectories, followed by TSCAN and Monocle DDRTree (`r ref("fig", "results_overview", "b")`). When we focused on the benchmark scores per trajectory type, slingshot was the only method that performed well across most trajectory types, followed by TSCAN and Monocle DDRTree. However, we found that several methods were specialised in predicting specific trajectory types; for example SCORPIUS for linear trajectories, reCAT for cycles, and Monocle DDRTree for trees. This was also reflected in the rankings of different metrics, with methods able to detect more complex trajectories, such as DPT, slingshot, TSCAN and Monocle DDRTree, performing better at our metric investigating topological similarity, while other methods, including SCORPIUS, slingshot and cellTree, were better at predicting the ordering and neighbourhood of the cells (`r ref("sfig", "metric_comparison")`).


<!-- §2: The stability and robustness of the evaluation/method ranking -->
```{r}
...
We further investigated the robustness of our evaluation across datasets and dataset sources. Method performance was moderately to highly correlated (0.5-0.9) between real and synthetic datasets (`r ref("fig", "results_overview", "b")` and `r ref("sfig", "real_synthetic_comparison")`), confirming the relevance of synthetic data, and the accuracy of the real datasets' gold standard. Furthermore, while performance was strongly dependent on individual datasets, the ordering of methods was still very robust, with most methods significantly outperforming the next method in the ranking `r ref("sfig", "performance_dataset_variability_combined")`. To further illustrate this, if we looked at the scores a random 10% subset of all datasets, the overall ordering was still highly correlated with the ordering on all datasets (average spearman's correlation of  `r round(correlation_tenth, 2)`), and the top method, slingshot, was the top method in `r scales::percent(top_method_rank_match)` of random dataset subsets. On the other hand, for nearly every method it was possible to find a subset of datasets on which it on average performed the best. For example, for SCOUP, a method ranked in the lower half, we found a subset of `r random_method_max_tasks_top`datasets on which it would be the top ranking method. Altogether, this demonstrates that almost every method can be the best on a particular set of datasets, and that a large set of datasets, such as those included in our evaluation, is necessary to robustly compare the overall performance of methods.
```{r}
add_sfig(
  figure_file(
  ...
  "Comparison of method performance between real and synthetic datasets across trajectory types. Given in the top-left corner of each panel is the Pearson's correlation."
)
```
...
add_sfig(
  figure_file(
    "performance_dataset_variability_combined.svg",
    "11-evaluation_robustness"
  ), 
...
add_sfig(
  ...
  "metric_comparison", 
  "Comparison of method performance between different metrics across trajectory types. For each row, a different pairwise comparison is given between two metrics."
)
```
...
add_sfig(
  ...
  "Score of methods containing particular algorithmic components. Shown are the algorithmic components present in four or more methods, and whether the performance of methods containing a certain component was significantly higher than all the other methods. NS: Not significant. *: corrected p-value < 0.05 "
)
```
...
While not directly related to the accuracy of the inferred trajectory, the quality of the implementation is also an important evaluation metric [@_doesyourcode_2018]. To do this, we scored each method using a checklist of important scientific and software development practices, including software packaging, documentation, automated code testing, and peer review. This allowed us to rank each method based on its user friendliness, developer friendliness, and potential broad applicability on new unseen datasets. 
...
```{r}
add_sfig(
  figure_file("modulenets.svg", experiment_id = "2-dataset_characterisation/3-synthetic"),
  "modulenets", 
...
```{r}
add_sfig(
  figure_file("score_aspects_v1.svg", experiment_id="3-score_rules/5-score_aspects"), 
  "score_aspects", 
...
add_stable(
  ...
  paste0("Overview of the trajectory inference methods included in this study, and several characteristics thereof. This table will be continuously updated ", url("https://goo.gl/FnsTxG", "online"), ".")
)
```
