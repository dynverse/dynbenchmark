...
In this study, we performed a comprehensive evaluation of  `r sum(methods$evaluated)` TI methods (`r ref("fig", "evaluation_overview", "a")`). The inclusion criterion for TI methods was primarily based on their free availability, the presence of a programming interface (`r ref("table", "implementations")`). The evaluation comprised three core aspects: (i) assessment of the accuracy and scalability of TI methods by comparing predicted trajectories with a gold standard, (ii) a quality control of the provided software and documentation, and (iii) combining both evaluations to create a set of practical guidelines for users.
...
<!-- Errors-->
Several methods obtained a high overall score despite having 5 to 10% of failed executions (e.g. Monocle DDRTree, SLICE, Wishbone), meaning these methods could rank even higher if not for the failed executions. Methods that do not scale well with respect to the number of cells or genes exceeded the pre-defined time or memory limits for the largest datasets (reCAT, SCOUP, StemID). For a few methods, the time or memory limits were exceeded too often, making the benchmarking results uncomparable to those of other methods (SCIMITAR, Ouija, Pseudogp).
...
### Method selection
We gathered a list of `r sum(implementations$is_ti, na.rm=TRUE)` trajectory inference methods (`r ref("table", "implementations")`), by searching in literature for "trajectory inference" and "pseudotemporal ordering", and based on two existing lists found online [@davis_awesomesinglecelllistsoftware_2018; @gitter_singlecellpseudotimeoverviewalgorithms_2018].
...
1) method performance highly depends on trajectory type
2) methods which are specific to a certain trajectory type generally perform better on datasets containing these trajectory types
3) the complexity of the topologies returned by the different methods is highly variable 
4) prior information doesn't increase performance in our settings