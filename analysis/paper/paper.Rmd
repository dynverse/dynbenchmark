...
Critical to the broad application of TI methods is that both the user's input and the method's output is standardised, so that users can easily and quickly try out different methods, compare their output, and run common analysis pipelines such as network inference [@aibar_scenicsinglecellregulatory_2017a] or module detection [@saelens_comprehensiveevaluationmodule_2018]. A common framework to analyse trajectories is especially important when the output of methods contradict, as they often do. We illustrate this in  `r ref("fig", "example_predictions")`. Here, we predicted trajectories on a datasets containing fibroblasts which were reprogrammed towards neurons and myocytes, a bifurcating trajectory [@treutlein_dissectingdirectreprogramming_2016]. The topology of all five methods differs greatly, with Slingshot underestimating the complexity of the topology, while AGA pseudotime and Monocle DDRTree overestimated the complexity, at least compared to what was expected in the data.
...
add_fig(
  ...
  caption_main = "An example gold standard and prediction.", 
  caption_text = "As an illustration to our practical guidelines"
)
```
