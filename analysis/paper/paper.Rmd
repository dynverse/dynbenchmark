...
## Method benchmark
An overview of the main results from this study is shown in `r ref("fig", "results_overview")`. 
...
## Real datasets
Overview:  (`r ref("sfig", "supp_real_data")`)
...
```{r}
add_sfig(
        {figure_file('supp_real_data.svg', 'evaluation_overview')},
"supp_real_data",  "Overview of the real data", 15, 7.5
)
```
...
Our workflow to generate synthetic data is based on the well established workflow used in the evaluation of network inference methods [@schaffter_genenetweaversilicobenchmark_2011; @marbach_wisdom_2012] and consists of four main steps: network generation, simulation, gold standard extraction and simulation of the scRNA-seq experiment (`r ref("sfig", "supp_synthetic_data")`). At every step, we took great care was taking to mimic real cellular regulatory networks as best as possible, while keeping the model simple and easily extendable. For every synthetic dataset, we used a random real dataset as a reference dataset (from those described earlier), making sure the number of variable genes and cells were similar.


<!-- REMOVED FROM THE MANUSCRIPT; MIGHT NEED TO BE MERGED WITH EXISTING TEXT HERE
These synthetic datasets were generated using a realistic model of gene regulation [@schaffter_genenetweaversilicobenchmark_2011], and for each synthetic dataset a real dataset was used as reference to match the statistical properties of this dataset as best as possible (`r ref("fig", "evaluation_overview", "d")`). We used both large and smaller real datasets, from a variety of platforms, organisms and containing all different trajectory types (`r ref("fig", "evaluation_overview", "f")` and `r ref("stable", "datasets")`). →


```{r}
add_sfig(
        {figure_file('supp_synthetic_data.svg', 'evaluation_overview')},
"supp_synthetic_data",  "Synthetic data generation workflow", 15, 7.5
)
```
### Network generation
...
Make into sentence
Check whether figure matches the 4 steps listed in the text
TODO
TODO
