...
These SDEs were simulated using the Euler–Maruyama approximation, with time-step $h = 0.01$ and noise strength $\eta = 2$.
...
We used a popular single-cell RNA-seq pipeline [@lun_stepbystepworkflowlowlevel_2016] which uses the scran and scater Bioconductor packages. The advantages of this pipeline is that it works both with and without spike-ins, and includes a harsh cell filtering which looks at abnormalities in library sizes, mitochondrial gene expression, and number of genes expressed. Furthermore, the pipeline can select the most highly variable genes.
## Evaluation metrics
## Trajectory types
