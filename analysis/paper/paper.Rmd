...
Depending on the output of a method, we used different strategies to convert the output to our common model:


* Linear trajectory methods usually return a single pseudotime value, which can be converted to our model using a single edge between two milestones
* Conversely, methods which assign each cell to a branch together with a pseudotime value and a branch network easily converted into our model
* Methods returning both a global pseudotime value and a probability for every end state were converted into our model by using one single begin milestone and an end milestone for every end state. The global pseudotime then determines the distance from the begin milestone, the rest of the cell's position is calculated by distributing the residual percentage over the end states, proportionally to the end state probabilities
Special conversions were necessary for certain method
* DPT: 
* Sincell
* SLICER
* SLICE
