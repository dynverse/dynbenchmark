...
![](`r figure_file("ti_evaluation_overview_v2.tmp.pdf", experiment_id="evaluation_overview")`)
...
We assessed whether the components of an algorithm can be predictive for method performance, using both random forest regression and statistical testing. Methods which include principal curves (which include Slingshot and SCORPIUS), k-means (which include Slingshot and several other top scoring methods) and some graph building (which include almost every top scoring method) tended to have a slightly higher performance (`r ref("sfig", "method_components_scores")` and `r ref("stable", "component_importance")`). On the other hand, methods using t-SNE and ICA for their dimensionality reduction were ranked lower, although this was not significant.
...
This is complex though to put into this figure :p. Have a look at 4-method_characteristics/wrapping.tmp.pdf I think it is the least crucial of all parts of the figure now, so.. But feel free to try it yourself :p
