...
trajectory_type_comparison <- milestone_networks %>%
  ...
  mutate(n=n/sum(n)) %>%
  ggplot(aes(trajectory_type_gold, trajectory_type_predicted)) +
  geom_tile(aes(fill=n)) +
  geom_tile(fill=NA, color="white", size=2) +
  geom_tile(aes(simplified, simplified, color=color), fill=NA, data=trajectory_types_simplified %>% filter(simplified %in% levels(milestone_networks$trajectory_type_predicted)), size=2) +
  scale_x_discrete(label_long("trajectory_type_gold"), label=label_long) +
  scale_y_discrete(label_long("trajectory_type_predicted"), label=label_long) +
...
complexity_difference_distribution <- milestone_networks %>%
  mutate(complexity_difference = complexity_predicted - complexity_gold) %>%
  {
    ...
    ) %>% mutate(trajectory_type_gold = factor(trajectory_type_gold, levels=c("all_trajectory_types", trajectory_types_simplified$simplified)))
  } %>%
  ...
  facet_grid(.~method_id, labeller=function(x) {tibble(method_id=methods$method_name[match(x$method_id, methods$method_id)])}) +
  ...
  annotate("segment", x = 0, xend = complexity_difference_limits[[2]] - bw, y = arrow_y, yend = arrow_y, colour = "#333333", arrow=arrow(type="closed", length=unit(0.1, "inches"))) +
  annotate("text", x = 0, y = arrow_y, colour = "#333333", label="More complex prediction", hjust=-0.2, vjust=-0.2) +
