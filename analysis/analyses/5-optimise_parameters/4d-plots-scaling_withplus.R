...
experiment("5-optimise_parameters/4-plots_withplus")
...
oi_cor <- oi %>%
  ...
  do({
    dat <- .
    prefix <- dat %>% select(method_name, method_short_name) %>% slice(1)
    df <- dat %>%
      select(time_method, ncell, nfeat) %>%
      mutate(
        ncell2 = ncell^2,
        nfeat2 = nfeat^2,
...
g <- ggplot(oi_cor) +
  geom_bar(aes(fct_rev(feature), rsq), stat = "identity") +
...
complexity <- oi_cor %>%
  ...
  ungroup() %>%
  mutate(
    feature = ifelse(rsq < .2, "nothing", as.character(feature)),
    feature2 = sub("_._", "_", feature),
    sign = ifelse(grepl("_", feature), c("p"="+", "t"="×")[gsub("[^_]*_?([^_]*)_?.*", "\\1", feature)], "~")
  )

comp_mat <- oi_cor %>% select(method_short_name, feature, rsq) %>% reshape2::acast(method_short_name~feature, value.var = "rsq")
...
for (m in mns) {
  cat("Processing ", m, "\n", sep = "")
  oim <- oi %>%
    filter(method_short_name == m, pct_errored == 0) %>%
    select(time_method, ncell, nfeat) %>%
    mutate(
      ncell2 = ncell^2,
      nfeat2 = nfeat^2,
  ...
  lm_fits <- oim %>%
    ...
    do({
      dat <- .
      fn <- unique(dat$feature)
      lm_fit <- lm(value ~ ., dat %>% select(-feature))
      sum <- summary(lm_fit)
      data_frame(feature = fn, intercept = lm_fit$coefficients[[1]], slope = lm_fit$coefficients[[2]], rsq = sum$r.squared, min = min(dat$value), max = max(dat$value))
    }) %>%
    ungroup()
