...
```{r}
source("analysis/analyses/3-score_rules/generation.R")
source("analysis/analyses/3-score_rules/perturbation.R")

experiment("score_rules")
```


```{r}
toys_blueprint <- tribble(
  ~trajectory_type, ~perturbator_id,
  "simple_linear", "gs",
  ...
  bind_rows(tibble(trajectory_type="simple_linear", perturbator_id=paste0("switch_", percs * 100))) %>%
  bind_rows(crossing(trajectory_type=trajectory_types, perturbator_id=paste0("change_network_", trajectory_types)) %>% mutate(ncells = 5)) %>% 
  rowwise() %>% 
  mutate(
...
toys <- toys_blueprint %>% slice(rep(1:n(), each=nreplicates)) %>% mutate(
  ...
  toy_category=paste0(trajectory_type, "-", perturbator_id),
...
toys$gs <- toys %>% split(seq_len(nrow(toys))) %>% parallel::mclapply(function(row) {
  generate_dataset(row$trajectory_type, row$ncells)
}, mc.cores=8)
...
metrics <- c("mean_R_nx", "auc_R_nx", "Q_local", "Q_global", "correlation", "isomorphic", "node_edit_score", "node_edge_edit_score", "mantel_pval", "ged")
...
qsub_run(
  ...
  qsub_config = override_qsub_config(num_cores = 24),
  qsub_environment = list2env(list(compare_toy, compare_gs_toy, toys)
)
