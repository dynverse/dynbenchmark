...
networks_synthetic <- map(topologies, dyntoy:::generate_milestone_network) %>% tibble(milestone_network = ., topology = topologies)
```

Add SCORPIUS predictions
```{r}
ti_methods <- list(ti_scorpius())
data <- dyntoy::generate_toy_datasets(c("linear"), num_replicates=1, num_cells=30, num_genes=10) %>% dynutils::extract_row_to_list(1)

results <- map(ti_methods, ~.$run_fun(data$counts))

networks_methods <- tibble(topology=map_chr(ti_methods, "name"), milestone_network=map(results, "milestone_network"))
```
