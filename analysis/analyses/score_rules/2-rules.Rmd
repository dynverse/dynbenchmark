...
metric_types[c("ged", "robbie_network_score", "isomorphic")] <- "structural"
...
## Grouping cells
```{r}
scores_summary %>% ggplot() + geom_point(aes(score, score_grouped, color=generator_id)) + facet_wrap(~score_id)

scores_summary %>% group_by(score_id) %>% summarise(
  cor = cor(score, score_grouped),
  rms = sqrt(sum((score - score_grouped)^2)),
  rms_shifted = sqrt(sum((score - mean(score) - score_grouped + mean(score_grouped))^2))
)
```
