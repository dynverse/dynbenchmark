...
implementation_ordering_plot <- implementations_evaluated %>%
  ...
  ggplot(aes(implementation_id, qc_score)) +
    ...
    scale_x_discrete(NULL, breaks=c(0, 100), expand=c(0, 0)) +
...
check_heatmap_plot <- implementation_qc_checks %>% 
  ...
  hline_check + hline_aspect +
    ...
    scale_fill_manual("Category", values=set_names(qc_categories$color, qc_categories$category), label=label_long, guide=guide_legend(ncol=2)) +
