...
implementation_ordering_plot <- implementations_evaluated %>%
  arrange(-qc_score) %>% 
  mutate(implementation_id = factor(implementation_id, levels=implementation_id)) %>% 
  ggplot(aes(implementation_id, qc_score)) +
...
saveRDS(implementation_qc_ordering_plot, figure_file("implementation_qc_ordering.rds"))
```
