...
g2 <- ggplot(methods_evaluated %>% mutate(ncitations = ifelse(ncitations == 0, 0.5, ncitations)), aes(date, ncitations)) +
    geom_smooth(span=2) +
    geom_point() +
    ggrepel::geom_label_repel(aes(label = name)) +
    cowplot::theme_cowplot() +
...
g3 <- ggplot(methods_evaluated %>% mutate(ncitations = ifelse(ncitations == 0, 0.5, ncitations)), aes(ncitations, qc_score)) +
    geom_smooth(span=2) +
    geom_point() +
    ggrepel::geom_label_repel(aes(label = name)) +
    cowplot::theme_cowplot() +
