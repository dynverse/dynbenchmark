...
# Score the tasks ---------------------------------
task_scores <- pmap(as.list(tasks), function(...) {
...
### Cell plotting ----------------
middle <- function(x1, x2) {x1 + (x2-x1)/2}
...
cell_plots <- pmap(as.list(tasks), function(...) {
  ...
  base_arrow <- ggplot() +
    # geom_point(aes(Comp1, Comp2), data=plotdata_groups, shape=21, stroke=4, size=3, alpha=0.3) +
    geom_segment(aes(from_Comp1, from_Comp2, xend=to_Comp1, yend=to_Comp2), data=plotdata_network,size=2, alpha=.5) +
    geom_segment(aes(x=middle_offset(from_Comp1, to_Comp1), y=middle_offset(from_Comp2, to_Comp2), xend=middle(from_Comp1, to_Comp1), yend=middle(from_Comp2, to_Comp2)), data=plotdata_network, arrow=arrow(type="closed", length=unit(0.2, "inches")), size=1) +
    geom_point(aes(Comp1, Comp2, fill=group_id), shape=23, stroke=1, data=plotdata_groups, size=3, color="black") +
    ggraph::theme_graph()

  base_cells <- ggplot() + geom_point(aes(Comp1, Comp2, color=group_id), data=plotdata_cells, alpha=0.4) + ggraph::theme_graph()

  plots$plot_network <- list(
    base_cells +
      geom_segment(aes(from_Comp1, from_Comp2, xend=to_Comp1, yend=to_Comp2), data=plotdata_network,size=2, alpha=.5) +
      geom_segment(aes(x=middle_offset(from_Comp1, to_Comp1), y=middle_offset(from_Comp2, to_Comp2), xend=middle(from_Comp1, to_Comp1), yend=middle(from_Comp2, to_Comp2)), data=plotdata_network, arrow=arrow(type="closed", length=unit(0.2, "inches")), size=1) +
      geom_point(aes(Comp1, Comp2, fill=group_id), shape=23, stroke=1, data=plotdata_groups, size=3, color="black")
  ...
  plots$plot_cells <- list(
    base_arrow + geom_point(aes(Comp1, Comp2, color=group_id), data=plotdata_cells)
  )
